...
Fields: `smid`, `startdate`, `enddate`, `price`, `duration`, `active`

> Do not request embedded `buyer` or `seller` user objects in paginated order calls. Live testing returned `Users - Invalid User ID Supplied`. Resolve usernames separately from local cache or a dedicated users lookup when needed.
