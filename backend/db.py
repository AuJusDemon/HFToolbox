...
def mark_token_dead(uid: str, dead: bool) -> None:
    with _db() as conn:
        try:
            conn.execute("ALTER TABLE users ADD COLUMN token_dead INTEGER NOT NULL DEFAULT 0")
        except Exception:
            pass  # column already exists
        conn.execute("UPDATE users SET token_dead=? WHERE uid=?", (1 if dead else 0, uid))
...
def clear_token(uid: str) -> None:
    """Invalidate a user's stored token (e.g. after 401 from HF)."""
    with _db() as conn:
        conn.execute("UPDATE users SET token='' WHERE uid=?", (uid,))


# ── Open contracts helper ───────────────────────────────────────────────────────

def get_open_contract_cids(uid: str) -> list:
    """Return CIDs of contracts still in an open state (Awaiting or Active)."""
    with _db() as conn:
        rows = conn.execute(
