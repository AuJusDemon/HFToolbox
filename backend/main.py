...
from dashboard_refresh import (
    ...
    contract_value as _contract_value,
    parse_brating_rows as _parse_brating_rows,
    store_bratings as _store_bratings,
)
...
async def _crawl_if_active(uid: str, token: str) -> bool:
    ...
    if idle_secs > IDLE_THRESHOLD:
        ...
        log.debug("Crawl: uid=%s idle %.0fs - marked stale until user returns", uid, idle_secs)
        return False
    await refresh_user_dashboard(uid, token)
    return True
                        ...
                        for uid in uids:
                            ...
                            try:
                                ...
                                if new_tok:
                                    ...
                                    try:
                                        await asyncio.wait_for(poll_autobump(uid, new_tok), timeout=120)
                                    except asyncio.TimeoutError:
...
def _repo_root() -> Path:
    mounted = Path(os.environ.get("HFT_MANIFEST_ROOT", "/manifest-src"))
    if mounted.exists():
        return mounted
