...
test('deep-linked job can open the immutable schedule editor', async ({ page }) => {
  ...
  await expect(editor.getByLabel('Calendar days').getByRole('button', { name:'Sunday' })).toHaveAttribute('aria-pressed', 'true')
  await expect(editor.getByLabel('Calendar time', { exact:true })).toHaveValue('10:00')
  await page.screenshot({ path:'test-results/bumper-calendar-buttons-mobile.png', fullPage:true })
  await page.setViewportSize({ width:1440, height:900 })
  await page.screenshot({ path:'test-results/bumper-calendar-buttons-desktop.png', fullPage:true })
