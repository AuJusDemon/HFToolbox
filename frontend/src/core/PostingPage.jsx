...
if (typeof document !== 'undefined' && !document.getElementById('hf-award-css')) {
  ...
  _lk.id = 'hf-award-css'; _lk.rel = 'stylesheet'
  _lk.href = 'https://hackforums.net/cache/themes/theme12/award.css'
...
function bbToHtml(raw, userGroups) {
  ...
  const _uimgSentinel = (url, size) =>
    `\x00UIMG\x00${encodeURIComponent((url || '').trim())}|${encodeURIComponent(size || '')}\x00`
  s = s.replace(/\[uimg=([^\]]+)\]([\s\S]*?)\[\/uimg\]/gi, (_, size, url) => _uimgSentinel(url, size))
  ...
  // [award=N] HF award sprite
  s = s.replace(/\[award=(\d+)\]/gi, (_, id) =>
    `<i class="award_sprite award_${id}" title="Award #${id}" style="display:inline-block;vertical-align:middle;margin:2px 3px"></i>`)
  ...
  // [x] — X/Twitter post embed preview
  s = s.replace(/\[x\]([\s\S]*?)\[\/x\]/gi, (_, url) => {
    const u = url.trim()
    return `<div style="border:1px solid #2f3336;border-radius:12px;padding:12px 14px;margin:8px 0;background:#0d0e10;max-width:520px;font-family:system-ui">` +
      `<div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">` +
      `<span style="color:#fff;font-size:17px;font-weight:900;line-height:1">𝕏</span>` +
      `<span style="color:#71767b;font-size:11.5px">Embedded Post</span>` +
      `</div>` +
      `<a href="${u}" target="_blank" rel="noreferrer" style="color:#1d9bf0;font-size:12px;word-break:break-all;text-decoration:none">${u}</a>` +
      `</div>`
  })

  // [aiq] — AI/LLM quote block (distinct from normal [quote])
  s = s.replace(/\[aiq\]([\s\S]*?)\[\/aiq\]/gi,
    '<div style="border-left:3px solid #00d4b4;margin:8px 0;background:rgba(0,212,180,.05);padding:8px 12px;border-radius:0 4px 4px 0">' +
    '<div style="font-size:9px;color:#00d4b4;font-family:Courier New,monospace;font-weight:700;text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px">▸ AI Quote</div>' +
    '<div style="color:#a0c4be;font-family:Courier New,monospace;font-size:12px;line-height:1.75;white-space:pre-wrap">$1</div>' +
    '</div>')

  // [soundcloud] — SoundCloud embed (real iframe in preview)
  s = s.replace(/\[soundcloud\]([\s\S]*?)\[\/soundcloud\]/gi, (_, url) => {
    const u = encodeURIComponent(url.trim())
    return `<iframe width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay" ` +
      `src="https://w.soundcloud.com/player/?url=${u}&color=%23ff5500&auto_play=false&hide_related=true&show_comments=false&show_user=true&show_reposts=false" ` +
      `style="border-radius:8px;margin:6px 0;display:block"></iframe>`
  })
...
function UimgEmbed({ url, size }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // Parse fragment: E2E1.<seed_b64url>.<iv_b64url>.<hmac_b64url>.<orig_frag>
        const hashPart = url.split('#')[1] || ''
        if (!hashPart.startsWith('E2E1.')) {
          setErrMsg('no E2E1 fragment'); setPhase('error'); return
        }
        const pieces = hashPart.slice(5).split('.')
        if (pieces.length < 4) {
          setErrMsg(`bad fragment (${pieces.length} parts)`); setPhase('error'); return
        }
        let seed, iv
        ...
        // pieces[2] = hmac, pieces[3] = gwKey (uploadimages.org's key, passed to their /api/image endpoint)
        ...
        let key
        try { key = await _e2eDeriveKey(seed) } catch(e) { setErrMsg('key derive: '+e.message); setPhase('error'); return }
        ...
        if (!cancelled) { setErrMsg('unexpected: '+e.message); setPhase('error') }
      }
    }
  ...
  if (size) {
    if (/^\d+[x×]\d+$/i.test(size)) {
      ...
      sizeStyle.width = w + 'px'; sizeStyle.height = h + 'px'
    } else if (/^\d+%$/.test(size)) {
  ...
  const pillBase = {
    ...
    fontFamily: 'var(--mono)', margin: '4px 0', verticalAlign: 'middle',
  }
  if (phase === 'loading') return (
    <span style={{ ...pillBase, background: 'rgba(75,140,245,.08)', border: '1px solid rgba(75,140,245,.2)', color: 'var(--acc)' }}>
      ⏳ loading image…
    </span>
  )
  if (phase === 'error') return (
    <span style={{ ...pillBase, background: 'rgba(232,84,84,.08)', border: '1px solid rgba(232,84,84,.2)', color: 'var(--red)' }}
          title={errMsg}>
      ⚠ uimg: {errMsg}
    </span>
  )
...
const HF_AWARDS = [
  [1,'Diamond'],[2,'White Hat Helper'],[3,'Speaker of the House'],[4,'Member of the Month'],
  [5,'Drunk'],[6,'Genius'],[7,'The Protector'],[8,'Dunce'],[9,'Turkey'],[10,'Asswipe'],
  [11,'Hustler'],[12,'Flamer'],[13,'Ninja'],[14,'Dark Inferno Wings'],[15,'Asskisser'],
  [16,'Militant'],[18,'Rich Bitch'],[19,'Star'],[20,'Black Hatter'],[21,'Gold Medal'],
  [23,'Gamer'],[24,'Frog'],[25,'Cool'],[26,'DaBomb'],[27,'Dancing Queen'],[28,'Pizza'],
  [29,'Silver Medal'],[30,'Bronze Tutorial Award'],[31,'GFX Guru'],[32,'Golden Content'],
  [34,'Jokester'],[35,'Emerald Donator'],[36,'Cat Lover'],[37,'Cuddly and Cute'],
  [38,'Mad Scientist'],[39,'Great Thinker'],[40,'Sapphire of Ub3r'],[41,'Dog Lover'],
  [42,'Support Feather'],[47,'Graphic Master'],[49,'Grammar Nazi'],[50,'Master Donator'],
  [51,'YouTube'],[52,'Fish Err Man'],[54,'Rat Expert'],[55,'The Alien'],[56,'Hot Dog'],
  [57,'Sushi Lover'],[58,'Businessman'],[59,'FPS Gawd'],[60,'Award Whore'],
  [61,'Liberty Reserve Head'],[62,'Qwazz Star'],[63,'Sticky Man'],[64,'Gavel of Dredd'],
  [65,'MRT Graduate'],[66,'Brony'],[67,'24k'],[68,'HackerCraft'],[69,'Spy'],
  [70,'Bitcoinage'],[71,'Gift'],[72,'Litecoinage'],[73,'Phreak'],[74,'HackerCraft Purple'],
  [75,'OMC'],[76,'Legend'],[77,'Top Banana'],[78,'Green Apple'],[79,'Legalize It'],
  [80,'Grand Amethyst'],[81,'Dogecoinage'],[82,'Royalty'],[83,'Benefactor'],[84,'DarkDash'],
  [85,'Green Clover'],[86,'Purple Clover'],[87,'Gold Clover'],[88,'Dstat Power God'],
  [89,'Instagrammy'],[90,'HackerCraft Blue'],[91,'Music Lover'],[92,'Ethereist'],
  [93,'Poke Ball'],[94,'Testified'],[95,'HF News Contributor'],[96,'Monero'],
  [97,'Dicey'],[98,'Eclipse'],[99,'ZECret'],[100,'XEMplary'],[101,'Rippled'],
  [102,'Stratisphere'],[103,'Fox'],[104,'BTCash'],[105,'Lounge Head'],
  [106,'Diamond of Hell'],[107,'Ace of Spades'],[108,'Lisking'],[109,'Qtum Umm'],
  [110,'PivX'],[111,'Wave Rider'],[112,'Xerotic Bot DNA'],[113,'Purple Gift'],
  [114,'Blue Gift'],[115,'Green Gift'],[116,'Cup of Joe'],[117,'Medal of Honor'],
  [118,'Youtube 2018'],[119,'Taco Taco Taco'],[120,'Grey Star'],[121,'Green Star'],
  [122,'Red Star'],[123,'Blue Star'],[124,'Gold Star'],[125,'Heart of HF'],
  [126,'Omners'],[127,'iHydra'],[128,'Cyber Diamond'],[130,'Money Money'],
  [131,'Everyday Ninja'],[132,'Cheap Skate'],[133,'SportsBook'],[134,'CyberMonday'],
  [135,'DIVIne Comedy'],[136,'Diamond of Gamblers'],[140,'Corona Virus'],
  [141,'Mr Monopoly'],[142,'Hackuman Killer'],[144,'K1NG'],[145,'Independence Egg'],
  [146,'Quickly Loved'],[147,'Astropink'],[148,'Ghoul'],[149,'Hack Friday'],
  [150,'Heart of Generosity'],[151,'LGBQT'],[152,'Euthanasia'],
  [153,"Maga Award"],[154,'The Plug'],[155,'Dainamic'],
  [156,'Binance Dance'],[157,'Olympians Trident'],[158,'Purple Haze'],
  [159,'Infamous Delta'],[161,'Ruby Heart Master Donator'],[162,'Emerald Heart'],
  [163,'Octoberfest 2021'],[164,'Bytes Bag'],[165,'Skull of Emptiness'],
  [166,'Skull of Agony'],[167,'Skull of Death'],[168,'Blackened Heart Gem'],
  [169,'Hot Wheels'],[170,'420 Award 2022'],[171,'420 Award 2022 Blue'],
  [172,'King Skull'],[173,'Green Bong'],[174,'True Gambler'],[175,'Warrior'],
  [176,'A Purple Hat'],[177,'Pint'],[178,'Octoberfest 2022'],
  [179,'Xmas 2022 Ornament 1'],[180,'Xmas 2022 Ornament 2'],[181,'Xmas 2022 Ornament 3'],
  [182,'Riddle King'],[183,'Youtube 2023'],[184,'Molten Sword'],
  [185,'Dynamite'],[186,'Lucky'],[187,'Shining Knight'],[188,'Black Hat Hacker'],
  [189,'White Hat Hacker'],[190,'Grey Hat Hacker'],[191,'Red Hat Hacker'],
  [192,'Blue Hat Hacker'],[193,'Green Hat Hacker'],[194,'King Decade'],
  [196,'Octoberfest 2023'],[197,'Gift 2023'],[198,'Gift in Yellow Box'],
  [199,"Sky Rocket"],[200,'Gold Coins'],[202,'Black Dank'],[203,'Solana'],
  [204,'Octoberfest 2024'],[205,'100'],[206,"Sox Sox"],[207,'Mister Peeks'],
  [208,'G.O.A.T'],[209,'Troll'],[210,'Golden Fleur-De-Lis'],[211,'Pulse Controller'],
  [213,'Xmas Ornament 2024'],[214,'Javascript Junkie'],[215,'Python Punk'],
  [216,'PHP Prodigy'],[217,'Money-'],[218,'Bounty Hunter'],[219,'Gaming Shooter 2025'],
  [220,'Chef Hat'],[221,'TikTok Trooper'],[222,'Flip Reaper'],
  [223,'H4CK3R Black Hat'],[224,'H4CK3R Red Hat'],[225,'H4CK3R Blue Hat'],
  [226,'Holy Leaf'],[227,'HF 18 Cake'],[228,'America'],[229,'Blazing Ember'],
  [230,'Stanley Fan'],[231,'Octoberfest 2025'],[232,'API Dev'],[233,'Earthling'],
]

function AwardPicker({ onSelect, onClose }) {
  const [q, setQ] = useState('')
  const ref = useRef(null)
  const filtered = q.trim()
    ? HF_AWARDS.filter(([, name]) => name.toLowerCase().includes(q.toLowerCase()))
    : HF_AWARDS
  ...
  return (
    <div ref={ref} style={{
      position:'absolute',zIndex:1000,top:'100%',left:0,marginTop:2,
      background:'var(--card)',border:'1px solid var(--b2)',borderRadius:6,
      boxShadow:'0 8px 32px rgba(0,0,0,.5)',width:324,padding:8,
    }}>
      <input autoFocus value={q} onChange={e=>setQ(e.target.value)}
        placeholder="search awards..." className="inp"
        style={{width:'100%',marginBottom:6,fontSize:11,padding:'3px 8px'}}
      />
      <div style={{
        display:'grid',gridTemplateColumns:'repeat(8,1fr)',gap:3,
        maxHeight:260,overflowY:'auto',padding:'2px 0',
      }}>
        {filtered.map(([id,name]) => (
          <div key={id} title={`#${id}: ${name}`}
            onClick={() => { onSelect(id); onClose() }}
            style={{display:'flex',alignItems:'center',justifyContent:'center',
              padding:3,cursor:'pointer',borderRadius:4,transition:'background 150ms'}}
            onMouseOver={e=>e.currentTarget.style.background='var(--hover)'}
            onMouseOut={e=>e.currentTarget.style.background='transparent'}>
            <i className={"award_sprite award_"+id} style={{display:'block'}}/>
          </div>
        ))}
        {filtered.length===0 && (
          <div style={{gridColumn:'1/-1',textAlign:'center',color:'var(--dim)',
            fontSize:11,padding:'10px 0',fontFamily:'var(--mono)'}}>no awards match</div>
        )}
      </div>
    </div>
  )
  ...
  return (
    <div style={{ position: 'sticky', top: 50, zIndex: 10 }}>
      ...
      <div style={{
        ...
        <Sep/>
        <Btn label="𝕏 Post" title="Embed an X/Twitter post" onClick={() => openModal(
          [{ key:'url', label:'X/Twitter post URL', placeholder:'https://x.com/user/status/...' }],
          ({url}) => { if(url) insert(`[x]${url.trim()}[/x]`) }
        )} />
        <Btn label="SoundCloud" title="Embed a SoundCloud track" onClick={() => openModal(
          [{ key:'url', label:'SoundCloud URL', placeholder:'https://soundcloud.com/...' }],
          ({url}) => { if(url) insert(`[soundcloud]${url.trim()}[/soundcloud]`) }
        )} />
        <Btn label="AI Quote" title="AI/LLM output quote block [aiq]" onClick={() => wrap('[aiq]','[/aiq]')} />
        <Sep/>
        ...
        <div style={{position:'relative',display:'inline-block'}}>
          <Btn label="Award" title="Insert an HF award" onClick={() => { saveSelection(); setShowAwardPicker(p=>!p) }}/>
          {showAwardPicker && (
            <AwardPicker
              onSelect={id => insert('[award='+id+']')}
...
function BBPreview({ message, title, userGroups, compact }) {
  ...
  // Odd-indexed segments are "url|size" pairs; even-indexed are raw HTML strings.
  const segments = useMemo(() => {
    const parts = html.split('\x00UIMG\x00')
    return parts.map((part, i) => {
      if (i % 2 === 1) {
        const sep = part.indexOf('|')
        const url  = decodeURIComponent(sep >= 0 ? part.slice(0, sep) : part)
  ...
  const inner = message ? (
    ...
    >
      {segments.map((seg, i) =>
        seg.type === 'uimg'
          ? <UimgEmbed key={i} url={seg.url} size={seg.size} />
...
function Composer({ onPosted }) {
  ...
  const [stashedReplies,    setStashedReplies]    = useState(null)
  ...
  const saveToDraft = async () => {
    ...
    try {
      ...
      setDraftFlash('ok')
      setTimeout(() => setDraftFlash(null), 3000)
    } catch {
      setDraftFlash('err')
      setTimeout(() => setDraftFlash(null), 3000)
    }
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!d.scheduled) {
        ...
        setStashedReplies(null); setMultiPostConfirm(false)
      }
      onPosted?.()
    } catch (e) {
      ...
      <div style={{ border: '1px solid var(--b1)', borderRadius: 4, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px',
          ...
          <button className={`tog${multiPost ? '' : ' off'}`} onClick={() => {
            if (multiPost) {
              if (reply1.trim() || reply2.trim()) {
                // Has content — ask first, don't touch anything yet
                setMultiPostConfirm(true)
              } else {
                // Empty — just turn off, nothing to lose
                setMultiPost(false)
                setReplyCount(1)
                setMultiPostConfirm(false)
              }
            } else {
              // Turning on — restore stash if available
              if (stashedReplies) {
                setReply1(stashedReplies.reply1)
                setReply2(stashedReplies.reply2)
                setReplyCount(stashedReplies.replyCount)
                setStashedReplies(null)
              }
              setMultiPost(true)
              setMultiPostConfirm(false)
            }
          ...
          {!multiPost && stashedReplies && (
            <button className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 10, padding: '1px 8px', color: 'var(--yellow)', borderColor: 'rgba(232,168,40,.3)' }}
              onClick={() => {
                setReply1(stashedReplies.reply1)
                setReply2(stashedReplies.reply2)
                setReplyCount(stashedReplies.replyCount)
        ...
        {/* Multi-post confirm banner — shown BEFORE anything is cleared */}
        {multiPostConfirm && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 14px',
            background: 'rgba(232,168,40,.09)', borderBottom: '1px solid rgba(232,168,40,.25)',
            fontSize: 12 }}>
            <span style={{ color: 'var(--yellow)', fontWeight: 600 }}>⚠ You have reply content.</span>
            <span style={{ color: 'var(--sub)', fontSize: 11 }}>Turn off multi-post? Your replies will be saved and can be restored.</span>
            <button className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 11, padding: '2px 10px', color: 'var(--yellow)', borderColor: 'rgba(232,168,40,.4)' }}
              onClick={() => {
                // Confirmed — now stash and clear
                setStashedReplies({ reply1, reply2, replyCount })
                setReply1(''); setReply2(''); setReplyCount(1)
                setMultiPost(false)
                setMultiPostConfirm(false)
              }}>
              Turn off
            ...
            <button className="btn btn-ghost" style={{ fontSize: 11, padding: '2px 10px' }}
              onClick={() => setMultiPostConfirm(false)}>
              Keep on
            </button>
          </div>
        )}
...
function DraftsPanel({ onSchedule, autoOpenId }) {
  const user       = useStore(s => s.user)
  const userGroups = user?.groups || []
  ...
  const [inviteLinkFlash, setInviteLinkFlash] = useState(false)
  ...
  const openEdit = useCallback(async (draft) => {
    ...
    setEditStashedReplies(null)
    setEditMultiPostConfirm(false)
  ...
  // Auto-open a specific draft when arriving via invite link
  const autoOpenFiredRef = useRef(false)
  useEffect(() => {
    if (!autoOpenId || autoOpenFiredRef.current || loading) return
    const target = [...myDrafts, ...sharedDrafts].find(d => d.id === autoOpenId)
    if (target) {
      autoOpenFiredRef.current = true
      openEdit(target)
    }
  }, [autoOpenId, loading, myDrafts, sharedDrafts, openEdit])
              ...
              <div style={{ borderTop: '1px solid var(--acc)', background: 'var(--bg)' }}>
                ...
                {showCollabPanel && isOwner && (
                  <div style={{ padding: '10px 12px', borderBottom: '1px solid var(--b1)', background: 'var(--s2)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, paddingTop: 8, borderTop: '1px solid var(--b1)' }}>
                      <button className="btn btn-ghost" style={{ fontSize: 11 }}
                        onClick={async () => {
                          try {
                            const r = await api.get(`/api/posting/drafts/${d.id}/invite-link`)
                            if (r?.url) {
                              await navigator.clipboard.writeText(r.url)
                              setInviteLinkFlash(true)
                              setTimeout(() => setInviteLinkFlash(false), 2000)
                            }
                          } catch {}
                        }}>
                        🔗 Copy Invite Link
                      ...
                      {inviteLinkFlash && (
                        <span style={{ fontSize: 10, color: 'var(--green)' }}>Link copied!</span>
                      )}
                      <span style={{ fontSize: 10, color: 'var(--dim)', marginLeft: 'auto' }}>
                        Only added collaborators can open this link.
                      </span>
                    </div>
                  </div>
                )}
                ...
                <div style={{ padding: '10px 10px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <button className={`tog${editMultiPost ? '' : ' off'}`} onClick={() => {
                      if (editMultiPost) {
                        if (editReply1.trim() || editReply2.trim()) {
                          // Has content — ask first
                          setEditMultiPostConfirm(true)
                        } else {
                          // Empty — just turn off
                          setEditMultiPost(false)
                          setEditReplyCount(1)
                          setEditMultiPostConfirm(false)
                        }
                      } else {
                        if (editStashedReplies) {
                          setEditReply1(editStashedReplies.reply1)
                          setEditReply2(editStashedReplies.reply2)
                          setEditReplyCount(editStashedReplies.replyCount)
                          setEditStashedReplies(null)
                        }
                        setEditMultiPost(true)
                        setEditMultiPostConfirm(false)
                      }
                    }} />
                    <span style={{ fontSize: 11, color: 'var(--sub)' }}>Multi-post mode</span>
                    <ImgBadge count={countImages(editMessage)} />
                    {!editMultiPost && editStashedReplies && (
                      <button className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 10, padding: '1px 8px', color: 'var(--yellow)', borderColor: 'rgba(232,168,40,.3)' }}
                        onClick={() => {
                          setEditReply1(editStashedReplies.reply1)
                          setEditReply2(editStashedReplies.reply2)
                  ...
                  {/* Draft multi-post confirm — shown BEFORE clearing anything */}
                  {editMultiPostConfirm && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px',
                      background: 'rgba(232,168,40,.09)', border: '1px solid rgba(232,168,40,.25)',
                      borderRadius: 4, fontSize: 11 }}>
                      <span style={{ color: 'var(--yellow)', fontWeight: 600 }}>⚠ You have reply content.</span>
                      <span style={{ color: 'var(--sub)' }}>Turn off multi-post? Your replies will be saved and can be restored.</span>
                      <button className="btn btn-ghost" style={{ marginLeft: 'auto', fontSize: 10, padding: '2px 8px', color: 'var(--yellow)', borderColor: 'rgba(232,168,40,.4)' }}
                        onClick={() => {
                          setEditStashedReplies({ reply1: editReply1, reply2: editReply2, replyCount: editReplyCount })
                          setEditReply1(''); setEditReply2(''); setEditReplyCount(1)
                          setEditMultiPost(false)
                          setEditMultiPostConfirm(false)
                        }}>
                        Turn off
                      ...
                      <button className="btn btn-ghost" style={{ fontSize: 10, padding: '2px 8px' }}
                        onClick={() => setEditMultiPostConfirm(false)}>
                        Keep on
                      </button>
                    </div>
                  )}
...
export default function PostingPage() {
  ...
  // Capture the draft ID from router state once on mount.
  // Router state is set by DraftJoinRedirect and never appears in the URL,
  // so there's nothing to clean up and no race condition possible.
  const [autoOpenDraftId] = useState(() => location.state?.openDraftId || null)

  useEffect(() => {
    if (autoOpenDraftId) setTab('drafts')
  }, [])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div className="card">
        ...
        <div className="card-body">
          ...
          {tab === 'drafts' && <DraftsPanel onSchedule={() => { setTab('scheduled'); setQueueKey(k => k+1) }} autoOpenId={autoOpenDraftId} />}
          {tab === 'replies' && (
